...
ExtMove* MovePicker::score(MoveList<Type>& ml) {
    ...
    for (auto move : ml)
    {
        ...
        {
            // histories
            m.value = 2 * (*mainHistory)[us][m.butterfly_index()];
            ...
            if (ply < LOW_PLY_HISTORY_SIZE)
                m.value += 8 * (*lowPlyHistory)[ply][m.butterfly_index()] / (1 + ply);
        }
        ...
        else  // Type == EVASIONS
        {
            if (pos.capture_stage(m))
                ...
                m.value = (*mainHistory)[us][m.butterfly_index()] + (*continuationHistory[0])[pc][to];
        }
    }
    return it;
}
