...
constexpr auto StartFEN = "rnbqkbnrrnbqkbnr/pppppppppppppppp/88/88/88/88/PPPPPPPPPPPPPPPP/RNBQKBNRRNBQKBNR w1 KQkqABab - 0 1";
...
UCIEngine::UCIEngine(int argc, char** argv) :
    ...
    // Try to auto-load Team Chess NNUE weights from executable's directory
    {
        ...
        TeamNNUE::global_nnue().load(dir + "team_chess_nnue.bin");
    }
}
...
void UCIEngine::loop() {
    ...
    do
    {
        ...
        else if (token == "loadnnue")
        {
            ...
            if (is >> nnuePath)
            {
                if (TeamNNUE::global_nnue().load(nnuePath))
                    sync_cout << "info string NNUE loaded from " << nnuePath << sync_endl;
                else
                    sync_cout << "info string Failed to load NNUE from " << nnuePath << sync_endl;
            }
            else
                sync_cout << "info string Usage: loadnnue <path_to_weights.bin>" << sync_endl;
        }
...
std::string UCIEngine::move(Move m, bool chess960) {
    ...
    {
        // Left half (files a-h): king goes to g or c
        // Right half (files i-p): king goes to o or k
        if (is_left_half(from))
            to = make_square(to > from ? FILE_G : FILE_C, rank_of(from));
        else
            to = make_square(to > from ? FILE_O : FILE_K, rank_of(from));
    }
