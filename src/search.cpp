...
void Search::Worker::ensure_network_replicated() {
    // No-op: NNUE disabled for team chess
}
...
void Search::Worker::iterative_deepening() {
    ...
    for (Color c : {WHITE, BLACK})
        for (int i = 0; i < BUTTERFLY_HISTORY_SIZE; i++)
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).butterfly_index()] << evalDiff * 9;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                ...
                history += 69 * mainHistory[us][move.butterfly_index()] / 32;
        ...
        if (capture)
            ...
            ss->statScore = 2 * mainHistory[us][move.butterfly_index()]
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).butterfly_index()] << scaledBonus * 243 / 32768;
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    Color us = pos.side_to_move();
    workerThread.mainHistory[us][move.butterfly_index()] << bonus;  // Untuned to prevent duplicate effort

    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.butterfly_index()] << bonus * 805 / 1024;
